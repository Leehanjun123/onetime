...
export default function PushNotificationManager({ onPermissionChange }: PushNotificationManagerProps) {
  ...
  const savePushSubscription = async (subscription: PushSubscription) => {
    try {
      ...
      const response = await fetch('https://onetime-production.up.railway.app/api/v1/push/subscribe', {
        method: 'POST',
        headers: {
  ...
  const unsubscribeFromPush = async () => {
    try {
      ...
      if (subscription) {
        ...
        const token = localStorage.getItem('token');
        if (token) {
          await fetch('https://onetime-production.up.railway.app/api/v1/push/unsubscribe', {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`
            }
  ...
  const testPushNotification = async () => {
    try {
      ...
      const response = await fetch('https://onetime-production.up.railway.app/api/v1/test-notification', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
